...
class AutomationRuleRepository:
    ...
    def _check_device_ownership(self, device_id: int, user_id: int):
        device = self.db.query(DeviceModel).filter(
            DeviceModel.id == device_id, DeviceModel.user_id == user_id).first()
        if not device:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN,
                                detail="El usuario no es propietario del dispositivo.")
    ...
    def list_by_user(self, user_id: int) -> List[AutomationRuleResponse]:
        rules = self.db.query(AutomationRuleModel).filter(
            AutomationRuleModel.user_id == user_id).all()
    ...
    def get_by_id(self, rule_id: int, user_id: int) -> Optional[AutomationRuleResponse]:
        rule = self.db.query(AutomationRuleModel).filter(
            AutomationRuleModel.id == rule_id, AutomationRuleModel.user_id == user_id).first()
        if not rule:
            return None
    ...
    def update(self, rule_id: int, rule: AutomationRuleUpdate, user_id: int) -> Optional[AutomationRuleResponse]:
        db_rule = self.db.query(AutomationRuleModel).filter(
            AutomationRuleModel.id == rule_id, AutomationRuleModel.user_id == user_id).first()
        if not db_rule:
            return None
        self._check_device_ownership(rule.trigger_device_id, user_id)
