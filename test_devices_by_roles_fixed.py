#!/usr/bin/env python3
"""
Test completo de endpoints de devices con diferentes roles de usuario - VERSIÓN CORREGIDA
Prueba permisos y funcionalidad para SuperAdmin y Usuario Regular
"""
...
class DeviceRolesTester:
    ...
    def test_devices_endpoints(self):
        ...
        print("🚀 === TESTING DEVICES ENDPOINTS BY ROLES - VERSIÓN CORREGIDA ===")
        print("=" * 80)
        ...
        for i, user_key in enumerate(authenticated_users):
            # Usar MAC address único para cada usuario
            test_device = SAMPLE_DEVICE.copy()
            test_device["mac_address"] = f"DD:EE:FF:00:11:{22 + i:02X}"
            test_device["name"] = f"Test Device {TEST_USERS[user_key]['role_name']}"
            
            result = self.test_endpoint(user_key, "POST", "/devices", test_device, 201)
            status = "✅" if result["success"] else "❌"
            print(f"{status} {result['user']}: Status {result['actual_status']}")
            
            # Mostrar detalles del error si falla
        ...
        if self.created_devices:
            ...
            for user_key in authenticated_users:
                # SuperAdmin y propietario pueden actualizar
                expected_status = 200
                if user_key != device_owner and TEST_USERS[user_key]["role_id"] not in [0, 1]:
                    expected_status = 403  # Forbidden para users que no son propietarios ni admin
        ...
        print("\n🔧 5. TESTING: GET /devices/type/17")
        print("-" * 50)
        for user_key in authenticated_users:
            result = self.test_endpoint(user_key, "GET", "/devices/type/17")
            status = "✅" if result["success"] else "❌"
            count = len(result["response_data"]) if result["response_data"] and isinstance(result["response_data"], list) else "N/A"
            print(f"{status} {result['user']}: Status {result['actual_status']} | Devices: {count}")

        # 6. TEST: GET /devices/location/{location_id}
        print("\n📍 6. TESTING: GET /devices/location/13")
        print("-" * 50)
        for user_key in authenticated_users:
            result = self.test_endpoint(user_key, "GET", "/devices/location/13")
            status = "✅" if result["success"] else "❌"
            count = len(result["response_data"]) if result["response_data"] and isinstance(result["response_data"], list) else "N/A"
            print(f"{status} {result['user']}: Status {result['actual_status']} | Devices: {count}")
        ...
        # 8. TEST: GET /devices/search (Buscar dispositivos) - CORREGIDO
        print("\n🔍 8. TESTING: GET /devices/search?name=Test (CORREGIDO)")
        print("-" * 50)
        for user_key in authenticated_users:
        ...
        if self.created_devices:
            ...
            for i, device_info in enumerate(self.created_devices[:2]):  # Solo primeros 2 para no eliminar todos
                ...
                for user_key in authenticated_users:
                    # SuperAdmin y propietario pueden eliminar
                    expected_status = 200
                    if user_key != device_owner and TEST_USERS[user_key]["role_id"] not in [0, 1]:
                        expected_status = 403  # Forbidden para users que no son propietarios ni admin
    ...
    def cleanup_remaining_devices(self):
        ...
        # Usar superadmin para cleanup
        superadmin_headers = self.get_headers("superadmin")
        
        for device_info in self.created_devices[:]:
            device_id = device_info["id"]
            try:
                response = requests.delete(f"{BASE_URL}/devices/{device_id}", headers=superadmin_headers)
    ...
    def generate_summary(self):
        ...
        print(f"   ✅ SuperAdmin tiene permisos completos")
...
def main():
    """Función principal"""
    print("🚀 Iniciando tests de roles para dispositivos...")
